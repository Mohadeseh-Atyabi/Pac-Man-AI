        ...
        while self.iterations > 0:
            ...
            for state in self.mdp.getStates():
                ...
                if len(possibleActions) == 0:
                    ...
                    stateValue = max(QValues)
                    newStateValue[state] = stateValue
            self.values = newStateValue
            self.iterations -= 1
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            ...
            if not self.mdp.isTerminal(state):
                stateValue = max([self.getQValue(state, action) for action in self.mdp.getPossibleActions(state)])
        ...
        for _ in range(self.iterations):
            ...
            for predecessor in predecessors[state]:
                if not self.mdp.isTerminal(predecessor):
                    QValue = max([self.getQValue(predecessor, action) for action in self.mdp.getPossibleActions(predecessor)])
